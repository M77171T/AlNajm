...
Below are some notes on how to build AlNajm Core for Windows.
...
To build AlNajm from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the AlNajm GitHub repository](https://github.com/alnajm/alnajm/)).
...
Note that for WSL v1 the AlNajm Core source path MUST be somewhere in the default mount file system, for
example /usr/src/alnajm, AND not under, for example, /mnt/d/alnajm.
...
```
Ensure that this toolchain can build for `posix` (else you may have compilation errors for AlNajm's dependencies):
```bash
...
way. This will install to `c:\workspace\alnajm`, for example:
```bash
make install DESTDIR=/mnt/c/workspace/alnajm
```
