AlNajm Core version 1.14.7 is now available from:

  <https://github.com/alnajm/alnajm/releases/tag/v1.14.7/>

This is a new minor version release, including enchancements to several RPC
methods and important security updates for AlNajm-Qt. AlNajm-Qt users on
...
AlNajm Core is extensively tested on Ubuntu Server LTS, macOS and Windows.
...
This release contains fixes for AlNajm Qt across all platforms that increase
security for end users.

### Disable BIP-70 payment server by default

To mitigate future potential risk inside AlNajm Qt, support for BIP-70 payment
...
inside AlNajm Qt. By default, incoming BIP-70 payment requests and files or
...
alnajm.conf or as an argument to AlNajm-Qt if absolutely needed. Please use
extreme caution when exercising this option.

This is step 1 in full deprecation of BIP-70, BIP-71 and BIP-72 inside AlNajm
...
* Updated the `rpcuser.py` script to improve the password and salt generation
  ...
  current authentication in `alnajm.conf` with new credentials generated with
...
Fee estimation was explicitly not ported or supported on AlNajm Core since
...
to support AlNajm parametrization.
...
* Updated the python `ltc-scrypt` module to a maintained fork (#3080), which
  can be found at https://github.com/alnajm/ltc-scrypt
